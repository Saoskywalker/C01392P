...
//Bit3 保留
//Bit2 SRST_F：软件复位标志位
//1：通过写SOFT_RST 寄存器产生软件复位//0：未収生软件复位
//Bit1 WDTR_F：WDT 复位标志位
//1：WDT 溢出产生复位//0：未发生WDT 溢出复位
//Bit0 POR_F：POR 上电复位标志位
//1：収生POR 上电复位（系统初次上电或系统电源的异常跌落恢复产生的重新上电）
//0：未収生POR 上电复位
...
//Bit6 S_TRG：ADC 软件触发位//1：触収ADC 转换//0：转换完成（硬件自动清0）
//Bit5-4 TRG_S<1:0>：触发方式选择位
//1x：TMR2 计数值匹配ADC_TR_R0 值后触収ADC，再PWM1_R0+ADC_TR_R0 后
//再次触収ADC,此处需注意的是整体的周期限制不要溢出。
...
//Bit7-0 ADC_TG_R0L<7:0>：TMR2 触収ADC 延时值低位
//ADC_TG_R0H TMR2 触发ADC 延时值高8 位
//Bit7-0 ADC_TG_R0H<7:0>：TMR2 触収ADC 延时值高位
...
//Bit7 TXST：UARTn 収送状态标志位
//1：UARTn 収送未完成//0：UARTn 収送已完成
//Bit6 TXBF：UARTn 収送缓冲区满标志位
//1：UARTn 収送缓冲区满//0：UARTn 収送缓冲区空
//Bit5-4 TX9S<1:0>：UARTn 収送第9 位数据格式选择位
//11：収送数据1
//10：収送数据0
//01：収送偶校验
//00：収送奇校验
//Bit3 STPS：収送STOP 位长度选择位
//1：収送2 位STOP 位//0：収送1 位STOP 位
//Bit2 TXDM：UARTn 収送数据格式选择位
//1：9 位数据//0：8 位数据
//Bit1-0 TXEN<1:0>：UARTn 収送使能位
//11：使能，収送空闲和収送寄存器空均产生中断
//10：使能，収送空闲产生中断
//01：使能，収送寄存器空产生中断
//00：关闭
//////////////////////////////////////////////////////////////
//UARTn_IE UARTn 中断使能寄存器（与IICn_IE，SPIn_IE 寄存器复用）
//Bit3 TX1_IE：UART1 収送中断使能位
//1：使能//0：禁止
//Bit2 RX1_IE：UART1 接收中断使能位
//1：使能//0：禁止
//Bit1 TX0_IE：UART0 収送中断使能位
//1：使能//0：禁止
